document.addEventListener('DOMContentLoaded', () => {
   ...
   */
   ...
   let columnsHolder = document.querySelector("#gallery .columns-holder");
   let columnsNum = ((myWidth/450)-1)
   for(let i=0; i<columnsNum; i++){
    columnsHolder.innerHTML+=`
    <ul class="cont${i}" style="column-width: 449.75px; margin: 0; padding: 0;" >
      <!-- ======= Images loaded from js ======= -->
    </ul>
    `
  }
  ...
  //let fileNames = ["20201113_004315","20210907_233422","20210907_233433","39515272_p0","46736519_p0","48466332_p0","48998760_p0","53003270_p0","54089272_p1","55982257_p0","56828146_p0","57203074_p0","57618220_p0"];
  shuffle(fileNames)

  fileNames.forEach(
    function(filename, index){
      let gallery = document.querySelector(`#gallery .cont${findLower()}`);
  ...
  function findLower(){
    let column = document.querySelector(`#gallery .cont0`);
    let min = column.offsetHeight
    let minId = 0
    for(let i=1; i<columnsNum; i++){
      column = document.querySelector(`#gallery .cont${i}`);
      if(min>column.offsetHeight){
        minId = i
      }
    }
    return minId
